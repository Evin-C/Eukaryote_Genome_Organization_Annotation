...
# 0) Inputs
# -----------------
wd <- "." # set your working directory where the pangenome_matrix.rds is located
...
ggplot(freq_data, aes(x = n_present, y = count, fill = type)) +
  ...
  scale_fill_manual(values = c("Genes" = "blue4", "Orthogroups" = "firebrick4")) +
  labs(
    x = "Number of genomes (1 = private, 5 = core)",
    y = "Count of genes / orthogroups",
    fill = NULL,
...
ggsave(file.path(wd, "Plots/pangenome_frequency_plot.pdf"), width = 10, height = 6)
...
#             1    Species-specific (1 genome)         5684      7528    Note: these numbers are only valid for this comparison.